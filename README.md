...
- AWS Clean Rooms：複数組織のデータを安全にコラボレーションする仕組みを、Entity Resolutionや差分プライバシーの観点も交えて掘り下げる。
- Amazon Verified Permissions：Cedarポリシーを使った細粒度認可を題材に、アプリケーションの権限設計をハンズオンで整理する。
- AWS Payment Cryptography：決済領域のHSM、鍵交換、CVV検証など、普段触れる機会が少ない暗号処理を噛み砕いて試す。
- AWS B2B Data Interchange：EDIデータの変換や取引先連携を、AWS上でどこまで低コードに扱えるかを確認する。
